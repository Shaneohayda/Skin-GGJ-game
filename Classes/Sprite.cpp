...
CSprite::CSprite(SDL_Renderer* passed_renderer, std::string FilePath, int x, int y, int w, int h, float *passed_CameraX, float *passed_CameraY, CCollisionRectangle passed_CollisionRect)
{
	...
	//If image could not be loaded print to command prompt
	if (image == NULL)
	{
	...
	//load image 
	CollisionImage = IMG_LoadTexture(renderer, "CollisionBox.png");

	//error message if image failed to load
	if (CollisionImage == NULL)
	{
		std::cout << "Unable to load " << "CollisionImage" << std::endl;
	}
